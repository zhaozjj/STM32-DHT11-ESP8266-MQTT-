...
void Beep_Init(void)
{
	...
	gpioInitStruct.GPIO_Pin = GPIO_Pin_13;						//将初始化的Pin脚
...
void Beep_Set(_Bool status)
{
	...
	GPIO_WriteBit(GPIOB, GPIO_Pin_13, status == BEEP_ON ? Bit_SET : Bit_RESET);		//如果status等于BEEP_ON，则返回Bit_SET，否则返回Bit_RESET
